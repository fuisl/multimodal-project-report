...
\begin{abstract}
Tropical infectious diseases pose significant diagnostic challenges, particularly in resource-limited settings where overlapping symptoms and 
fragmented clinical data increase uncertainty and delay treatment. Although recent advances in large language models (LLMs) and multimodal models (LMMs) 
offer potential for integrating text, medical images, and structured data, current GenAI systems remain limited in domain-specific adaptation, 
...
we develop and comparatively evaluate three approaches for multimodal clinical decision support: Retrieval-Augmented Generation (LightRAG), 
domain-specific fine-tuning using NVIDIA NeMo, and a hybrid architecture combining both strategies. We construct a standardized performance is assessed 
using an automated evaluation framework based on RAGAS, and interpretability is enhanced through a LangGraph interface with step-level reasoning monitoring. 
